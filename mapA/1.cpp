...
void tester(void) {
	//	test: constructor
	std::map<Integer, std::string, Compare> map;
	//	test: empty(), size()
	...
	for (int i = 0; i < 100000; ++i) {
		...
		if (i & 1) {
			...
			auto result = map.insert(std::pair<Integer, std::string>(Integer(i), string));
	...
	for (int i = 0; i < (int)map.size(); ++i) {
		std::map<Integer, std::string, Compare> copy(map);
		map.clear();
	...
	std::map<Integer, std::string, Compare>::const_iterator const_iterator;
	...
	std::map<Integer, std::string, Compare>::iterator iterator;
	iterator = map.end();
	while (true) {
		std::map<Integer, std::string, Compare>::iterator peek = iterator;
		if (peek == map.begin()) {
			std::cout << std::endl;
			break;
		}
